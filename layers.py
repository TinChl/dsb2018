...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train = True):
        ...
        if len(pos_output)>0:
            ...
            ph, pw, py, px = pos_output[:, 1], pos_output[:, 2], pos_output[:, 3], pos_output[:, 4]
            lh, lw, ly, lx = pos_labels[:, 1], pos_labels[:, 2], pos_labels[:, 3], pos_labels[:, 4]

            regress_losses = [
                self.regress_loss(ph, lh),
                self.regress_loss(pw, lw),
                self.regress_loss(py, ly),
                self.regress_loss(px, lx),]
            ...
            # classify_loss = self.classify_loss(torch.cat((pos_prob, neg_prob), 0), torch.cat((pos_labels[:, 0], neg_labels + 1), 0))
    ...
    def set_num_hard(self, num_hard):
        self.num_hard = num_hard
