...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train(train_loader, net, loss, epoch, optimizer, get_lr, get_num_hard, args.save_freq, save_dir)
        validate(val_loader, net, loss)

def train(data_loader, net, loss, epoch, optimizer, get_lr, get_num_hard, save_freq, save_dir):
    ...
    #loss.set_num_hard(get_num_hard(epoch))
...
def test(data_loader, net, get_pbb, save_dir, config, test_set):
    ...
    for i_name, (data, target, coord, nhw) in enumerate(data_loader):
        s = time.time()
        ...
        output = split_comber.combine(output,nhw=nhw)
        if isfeat:
